class CNTROriannaVampireNPC extends CNTRCommonNPC {
    ...
    timer function morphMe( time : float , id : int) {
        ...
        if (components.Size() == 0) {
            NTR_notify("CNTROriannaVampireNPC: [ERROR] Not found morph managers for " + this);
        }
        for (j = 0; j < components.Size(); j += 1) {
            ...
            if (manager) {
                if (NTR_morphRatio.Size() < 1 || NTR_blendTime.Size() < 1) {
                    NTR_notify("CNTROriannaVampireNPC: [ERROR] NULL morph ratio/time stack!");
                }
                NTR_notify("CNTROriannaVampireNPC: [Info] Current morph ratio: " + manager.GetMorphBlend());
                manager.SetMorphBlend( NTR_morphRatio[0], NTR_blendTime[0] );
                NTR_notify("CNTROriannaVampireNPC: [OK] Morph component: " + manager + " to <" + NTR_morphRatio[0] + "> in " + NTR_blendTime[0] + " sec");
